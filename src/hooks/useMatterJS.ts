...
import popSoundSrc from "../assets/fruit/pop.mp3";
import pop2SoundSrc from "../assets/fruit/pop2.mp3";
import { FruitImg } from "../assets";

const { Engine, Render, World, Mouse, MouseConstraint } = Matter;
const frameInterval = 1000 / 60; // 60fps
const getImgUrl = (fruit: Fruit) => FruitImg[fruit];
...
const prevPosition = { x: getRenderWidth() / 2, y: 50 };
  ...
  Matter.Events.on(engine, "collisionStart", (event) => {
    ...
    pairs.forEach((pair) => {
      ...
      if (labelA === labelB) {
        ...
        // 과일이 합쳐질 때 사운드 효과
        const popSound = new Audio(pop2SoundSrc);
        popSound.play();
